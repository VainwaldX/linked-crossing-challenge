...
bool My_window::fichier_ouvert(const std::string& file_name) {
...
void My_window::restart_clicked() {
    ...
    //
    init_reset();
    set_jeu(previous_file_name); // réinitialise avec le dernier fichier enregistré
    //
}
...
void My_window::start_clicked() {
    ...
    } else {// if (appel pour obtenir le statut du jeu !== ON_GOING)
...
void My_window::dialog_response(int response, Gtk::FileChooserDialog *dialog) {
    ...
    case OPEN:
        if (file_name != "") {
	        // remplacer affichage par votre code
			std::cout << file_name <<"  " << __func__ << endl;
			//
			set_jeu(file_name);
			//
            dialog->hide();
...
void My_window::update() {
	...
	game_state* etat = get_state();
	
    if (etat->status != ONGOING) {
        buttons[B_SAVE].set_sensitive(false);
...
void My_window::update_infos() {
	game_state* etat = get_state(); // récupère etat dans jeu.cc
	
    std::cout << __func__ << std::endl;
    
    info_value[0].set_text(std::to_string(etat->score));
    info_value[1].set_text(std::to_string(etat->particules.size()));
    info_value[2].set_text(std::to_string(etat->faiseurs.size()));
    info_value[3].set_text(std::to_string(etat->chainejeu.get_chaine().size()));
}
...
void My_window::set_jeu(const std::string& file_name) {
	// remplacer affichage par votre code
	std::cout <<  __func__ << endl;
	//
	lecture(file_name);
